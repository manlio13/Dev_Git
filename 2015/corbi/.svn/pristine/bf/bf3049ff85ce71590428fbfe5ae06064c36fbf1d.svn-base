unit AnaAtl;   // Unità per il carico dell'anagrafe atleti
...
var
  Form3: TForm3;
  atlflg:Boolean=True; // Quando è True il form3 è closed
...
// per monitorare il form close quando si è aggiunto un record
...
begin
   if inserisco=True then  //se ha inserito un record
     ...
     begin
      ...
      atlflg:=True;  //per consentire di monitorizzare che Form3 è closed
      Form2.Visible:=True;
      Form3.Release;
     end;
end;
...
begin
    ...
    i:=0;          //determina se nome o/e cognome è in ricerca
     if edit1.text<>'' then
      begin
       i:=i+1;
       chi[i]:=1;
