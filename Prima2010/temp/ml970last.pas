{PROGRAMMA DI GESTIONE DELLE SCADENZE Ricorda 3.02  M.L. 1999  lasc@iol.it}
unit Ml970124;
...
var
  ...
  numdato,kk,p,b,a,canc,errore,finale,cntr :integer;
...
exit else
  ...
  if dummy='03/03/03'then //se ha una globale
if fileexists('.\ricorda1.dat') then
   begin
reggy:=true;
exit;
  ...
  if (strtodate(dummy)-dummy1=396)then //se dummy è 02/02/02
    ...
    begin
    showmessage('Errore inatteso, reinstallare o mailto: lasc@iol.it');
    halt;
    end;
end;
...
listbox1.clear;
for k:=0 to 999 do
num[k]:=0;//conta il numero di dati
...
finale:=k1-1;//perchè k si incrementa ogni fine ciclo
closeFile(F);
end;
...
//{Ricorda 3.02.38 sorgente	3.10.99 }
