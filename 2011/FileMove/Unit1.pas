...
{$R *.dfm}
  ...
  if fileexists(dir+'filemove.ini')=false then
   ...
   //   myIni:=Tinifile.Create(dir+'filemove.ini');
      if (MyIni.ReadString('extension','ext','')='') then
      showmessage('INI file to be corrected !');
      begin
      if Dialogs.MessageDlg('Change last used settings ?',
      mtConfirmation, [mbYes, mbNo], 0, mbNo) = mrYes then
        ...
        goto Again;
          ...
          if (MyIni.ReadString('extension','ext','')='') then
             begin
               ShowMessage('Sorry INI file has not been corrected.'#13#10'Cannot procede!');
               Form1.Close;
