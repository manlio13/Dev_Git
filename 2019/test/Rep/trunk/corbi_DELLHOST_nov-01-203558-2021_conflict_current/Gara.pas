...
begin   //dovrebbe servire per scrivere yes nel campo DQ, quando c'è
...
begin
   ...
   begin
    ...
    filtGa:=True;
    if (quanti>0) AND (mncti=False)  then  //Continua solo se non esiste già il buffer
      begin
   ...
   ABSTable14.Edit;
    while not ABSTable14.eof do
     begin
       ...
       Checkbox2.Checked:=False;
       Form10.Hide;
       ShowMessage('La tabella Gare Generale è stata aggiornata.');
       Button5.Click;
end;
...
begin
  if FiltGa=False then
    begin
      ShowMessage('Non si può editare lo storico.'+#13+'Inserisci prima il codice Gara da editare') ;
      Exit;
    end else
  begin

   if inserisco7=True then Exit;   //quando ho già inserito
   dbgrd7.ReadOnly:=False; //tempo è caricato da Temp con i dati che si vogliono trasferire
   ...
   if (tempo<>'') and (checkbox1.Checked=False) then  //Inserimento Tempi della disciplina
     ...
     TBooleanField(Form6.ABSTable14.FieldByName('MANL')).DisplayValues:='Si;No';
      if (rab1 = True) then    //rab1 è TempoA
        begin
      ...
      if (rab2 = True) then   //rab2 è TempoB
        begin
      ...
      if (rab3 = True) then   //rab3 è Penalità
        begin
    ...
    if (tempo='') and (checkbox1.Checked) then    //per inserire il DQ
      begin
         ...
         if ABSTable14.FieldByName('DQ').AsBoolean=True then
           begin
             if MessageDlg('Esiste già un DQ, vuoi cancellarlo ?',
              mtconfirmation, [mbYes,mbNo], 0,mbNo) = mrYes then
     ...
     if (mancante>0) AND (mncat=True ) then    //per inserire un mancante CAT
      begin
         ...
         if ABSTable14.FieldByName('CAT').AsInteger>0 then
             ...
             if MessageDlg('Esiste già una Categoria, vuoi sostituirla ?',
              mtconfirmation, [mbYes,mbNo], 0,mbNo) = mrNo then Exit;
      ...
      begin
         ...
         if ABSTable14.FieldByName('NMPET').AsInteger>0 then
             ...
             if MessageDlg('Esiste già un numero di Pettorale, vuoi sostituirlo ?',
              mtconfirmation, [mbYes,mbNo], 0,mbNo) = mrNo then Exit ;
      ...
      begin
         ...
         if ABSTable14.FieldByName('NMPOS').AsInteger>0 then
             ...
             if MessageDlg('Esiste già un numero di Posizione, vuoi sostituirlo ?',
              mtconfirmation, [mbYes,mbNo], 0,mbNo) = mrNo then Exit ;
      ...
      begin
         ...
         if ABSTable14.FieldByName('MANL').AsBoolean=True then
             if MessageDlg('Esiste già la Manleva, vuoi Cancellarla ?',
              mtconfirmation, [mbYes,mbNo], 0,mbNo) = mrNo then Exit ;
