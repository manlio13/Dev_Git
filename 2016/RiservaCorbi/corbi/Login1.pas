  ...
  if FileExists(dir+'corbi.ABS')=False then  //se non c'è il database
    begin
      ...
      begin             // se c'è il database
       ...
       if (Form2.ABSTable1.FieldByName('Password').IsNull) then  //se non c'è ancora la pw  CREARE LA PW
        begin
              ...
              if cnvstr=riconosce then
               ...
               begin
                if cicla=True then  //ha sbagliato per la seconda volta
                 ...
                 begin
                  MessageDlg('ATTENZIONE - Password errata', mtError,[mbOK],0 );
                  pwstr:='';
                  cnvstr:='';
                  edit1.text:='';
                  cicla:=True ;
