...
uses
  ...
  System.Classes, Vcl.Graphics,Vcl.Controls, Vcl.Forms, Dialogs, Vcl.StdCtrls,
  System.IniFiles, Vcl.FileCtrl, System.UITypes;
...
begin
  ...
  if phase=0 then
   ...
   myIni:=TIniFile.Create(dir+'SpostaFile.ini'); // crea l'oggetto inifile
                          //altrimenti provoca un accesserror  per mancanza di
                          //definizione dell'oggetto
    if fileexists(dir+'SpostaFile.ini')=false then
    begin //se non esiste da la possibilità di creare il file ini
    phase:=phase+1;
    Again:
    Form1.Visible:=true;
      // SetWindowPos(Handle,  HWND_TOPMOST,  left,   0,   0,   0,
      // SWP_NOACTIVATE or SWP_NOMOVE or SWP_NOSIZE); //on top
    label1.visible:=true;
    ...
    button2.Visible:=True;
     showmessage('Fill the form and then click on "writeIni"');
      ...
      begin
        ...
        showmessage('Correggi il file ini');
        goto Again;
      end else
