unit Unit1; // Diesel4     versione 4.1.0.230   22/09/20
  ...
  edit20.Text:=zerostr;
       ...
       //legge il file ini e proietta i dati sulla finestra
         ...
         if ((MyIni.Readstring('settings','alarm','')<>'') AND
          ...
          then    //esistono le ore relative al consumo da pieno
            label26.Caption:=('At Hours: '+MyIni.Readstring('data','HrsCalc','')) else
            label26.Caption:='';
      ...
      // inizializza il database e verifica l'esistenza di *.ABS
       try
        ...
        GetAppVersionStr;
        caption:=caption + ' ver: '+versione+'            Help press F1';
end;
...
procedure TForm1.Edit16KeyPress(Sender: TObject; var Key: Char);//consumo f(RPM)
   var
fcalc :double;
flo16:double; //numero di giri inserito
begin
      ...
      begin                     //verifica se combobox1 seleziona il motore
        ...
        begin
          ...
          if combobox1.ItemIndex =4 then  //D1-30 VolvoPenta
            begin
              fcalc :=0.11*intpower(flo16,3)+0.54*intpower(flo16,2)-1.23*flo16+1.25;
              edit15.Text:= FormatFloat('#.##',fcalc);
            end;
...
begin
    ...
    if atk<=tk*alarm then
     ...
     begin
       ...
       SendMessage(PB.handle, PBM_SETBARCOLOR, 0,clAqua) ;
...
begin
     if (Label11.Color=clRed)and (Edit6.Color=clLime) then
     begin
      Label11.Color:=clYellow ;
      Label11.font.color:=clWindowText;
      end else
  if Edit6.Color= clLime then
   begin
        Label11.Color:=clRed;
...
begin
   ...
   begin
    ...
    if (Edit18.Text<>'')  then    //consumo calcolato dal pieno
       cn:= strtofloat(edit18.Text) else   //cn è il consumo orario
      begin
    ...
    if (RadioButton3.Checked=True) then
     if (Edit15.Text<>'')  then    //consumo calcolato dai giri
       cn:= strtofloat(edit15.Text) else   //cn è il consumo orario
    begin
        ShowMessage('Data missing, cannot use this method !');
        Exit;
    end;
    ...
    radiobutton7.Checked:=True;
    radioButton1.Checked:=True;
    myini.WriteString('settings','rate',floatToStr(cn)); //scrive su ini il nuovo rate di consumo
    cn:=strtofloat(edit2.Text); //ricrea la costante del consumo
